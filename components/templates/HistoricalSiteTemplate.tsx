...
export default function HistoricalSiteTemplate({
  siteName,
  overview,
  mainImage,
  additionalImages,
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 mb-8">
          <div>
            <h1 className="text-4xl font-bold mb-4 gradient-text">{siteName}</h1>
          ...
          <TabsContent value="attractions">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {attractions.map((attraction, index) => (
                <Link
                  key={index}
                  href={`/attraction/${attraction.name.toLowerCase().replace(/ /g, "-")}`}
                  className="group"
                >
          ...
          <TabsContent value="restaurants">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {restaurants.map((restaurant, index) => (
                <Link
                  key={index}
                  href={`/restaurant/${restaurant.name.toLowerCase().replace(/ /g, "-")}`}
                  className="group"
                >
                  <Card className="overflow-hidden h-full transition-all duration-300 hover:shadow-lg hover:shadow-primary/50 hover:-translate-y-1">
                    <div className="relative h-48 overflow-hidden">
                    ...
                    <CardContent>
                      ...
                      <p className="text-sm text-primary mt-2">{restaurant.priceRange}</p>
                    </CardContent>
                  </Card>
                </Link>
              ))}
